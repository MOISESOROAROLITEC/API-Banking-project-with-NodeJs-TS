...
export const userCreate = async (req: Request, res: Response) => {
  try {
    ...
    const isValidate = createUserValidator.validate(req.body).error?.details[0]
      .message;
    if (isValidate) {
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      if (error.code === "P2002") {
        return res
          .status(401)
          .json({ message: `Cette adresse email est déjà utilisé` });
      }
    }
...
export const createSuperAdmin = async (data: {
  ...
  try {
    ...
    const isValidate = createUserValidator.validate({
      name,
      email,
      password,
      role,
...
export const changePassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!authorization || !authorization.startsWith("Bearer ")) {
      return res
        .status(400)
        .json({ message: "Faite la requete avec un Bearer token" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Le mot de passe a bien été changé" });
  } catch (error) {
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const isValidate = loginValidator.validate(req.body).error?.details[0]
      .message;
    if (isValidate) {
      return res.status(400).json({ message: isValidate });
    }
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    const isValidate = updateUserValidator.validate(req.body).error?.details[0]
      .message;
    if (isValidate) {
      return res.status(400).json({ message: isValidate });
    }
    ...
    if (oldPassword && newPassword && confirmPassword) {
      if (newPassword != confirmPassword) {
        return res.status(400).json({
          message:
            "Le nouveau mot de passe ne correspond pas au mot de pass de confirmation",
        });
      }
      const oldPasswordMatchWithHash = comparePassword(
        oldPassword,
        user.password
      );
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      if (error.code === "P2025") {
        return res.status(404).json({
          message: "Cet utilisateur est introuvable dans la base de données",
        });
      } else if (error.code === "P2002") {
        const target: string[] = error.meta!["target"] as string[];
        return res
          .status(400)
          .json({ message: `Un compte avec ce ${target[0]} existe déjà` });
      }
    }
    return res.status(500).json({ message: "server was crashed", error });
  }
...
export const getUserInformations = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ ...user, token: undefined, password: undefined, id: undefined });
  } catch (error) {
    if (error instanceof PrismaClientKnownRequestError) {
      if (error.code === "P2025") {
        return res.status(404).json({
