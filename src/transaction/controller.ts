...
function getTransactionValidationType(
...
export const creditAccount = async (req: Request, res: Response) => {
  try {
    ...
    let accoutToCredit: Account | SubAccount | null =
      await prisma.account.findUnique({ where: { iban: iban } });
    if (!accoutToCredit?.iban) {
      accoutToCredit = await prisma.subAccount.findUnique({
        where: { iban: iban },
      });
      if (!accoutToCredit) {
        return res.status(404).json({
          massage: `Ce iban : '${iban}' ne correspond à aucun compte ou sous compte `,
        });
      } else {
        accoutToCredit = await prisma.subAccount.update({
          where: { iban: iban },
          data: { balance: accoutToCredit.balance + +amount },
        });
      }
    } else {
      accoutToCredit = await prisma.account.update({
        where: { iban: iban },
        data: { balance: accoutToCredit.balance + +amount },
      });
    }
  ...
  try {
    if (!isTypeValide(req.body.transactionType)) {
      return res.status(400).json({
        message: `Le type de la transaction 'transactionType' est incorrect. les types acceptés sont : ${[
          ...availableTransactionTypes,
        ]}`,
      });
    }
    const { accountEmmiterIban, amount, transactionType } = req.body;
    const accountReciverIban: string =
      transactionType != "transfert" ? "null" : req.body.accountReciver;
    var transactionTypeValidator =
      getTransactionValidationType(transactionType);
    const isValidate = transactionTypeValidator.validate(req.body).error
      ?.details[0].message;
    if (isValidate) {
      return res.status(400).json({ message: isValidate });
    }
    let emmiterAccount: Account | SubAccount | null =
      await prisma.account.findUnique({ where: { iban: accountEmmiterIban } });
    if (!emmiterAccount) {
      emmiterAccount = await prisma.subAccount.findUnique({
        where: { iban: accountEmmiterIban },
      });
    }
    if (!emmiterAccount) {
      return res
        .status(404)
        .json({ message: `Aucun compte n'a ce IBAN : ${accountEmmiterIban}` });
    }
    ...
    if (!(transactionType === "credit")) {
      ...
      if (!(transactionType === "debit")) {
        reciverAcount = await prisma.account.findUnique({
          where: { iban: accountReciverIban },
        });
        if (!reciverAcount) {
          reciverAcount = await prisma.subAccount.findUnique({
            where: { iban: accountReciverIban },
          });
        }
        if (!reciverAcount) {
    ...
    try {
      await prisma.account.update({
        where: { iban: emmiterAccount.iban },
    ...
    return res.status(201).json({
      ...
      accountEmmiterIban,
      amount,
      currency: emmiterAccount.currency,
      accountReciverIban,
      createAt,
      updateAt,
    });
...
export const getOneTransaction = async (req: Request, res: Response) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ message: `you should provide id and it should be a number` });
    }
    const transaction = await prisma.transaction.findUnique({ where: { id } });
    if (!transaction) {
      return res
        .status(404)
        .json({ message: `nothing transaction with id : ${id}` });
    }
...
export const changeStatus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (user.role != "admin") {
      return res.status(400).json({
        message:
          "Seul les administrateur peuvent changer l'etat d'une transaction",
      });
    }
    if (newStatus !== "Accepté" && newStatus !== "Rejeté") {
      return res.status(400).json({
        message:
          "Le status 'newStatus' est invalide, entrez : Accepté ou Rejeté ",
      });
    }
    if (!id) {
      return res
        .status(400)
        .json({ message: "L'identifiant 'id' de la transaction est requis" });
    }
    ...
    if (
      ...
      return res.status(404).json({
        message: `Aucune transaction ne correspond à ce identifiant : ${id}`,
      });
    }
    return res.status(500).json({ message: "server was crashed", error });
  }
...
export const getUserTransactions = async (req: Request, res: Response) => {
  try {
    ...
    let transactionStatus =
      req.query.status === "false" ? undefined : (req.query.status as string);
    let typeOfAccount =
    ...
    const accounttrans = await prisma.transaction.findMany({
      where: {
        ...
        OR: [
          {
            accountEmmiterIban: typeOfAccount
